...
#if CONFIG_VIDEO_USE_FS_DC
// Use ~71% of the full range of DAC output - well within its linear region.
// Video scaled to 128 levels.
// Requires a series 100 ohm resistor to scale the voltage back down to 0-1V
#define DAC_LEVEL_SYNC 0 // sync pulse level 0V
#define DAC_LEVEL_BLACK 56 // black level 0.3V @ 2.36V
#define DAC_LEVEL_WHITE 182 //white level 1V @ 2.36V
#else
// No extra voltage divider on DAC output (video scaled to 55 levels)
#define DAC_LEVEL_SYNC 0 // sync pulse level 0V
...
static void setup_video_dac(void)
{
    ...
    ESP_LOGI(TAG, "DAC output on GPIO26 (DAC_CHANNEL_2)");
